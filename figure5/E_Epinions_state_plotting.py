...
xs_our=load_dict('E_Epinions_xs_our_'+str(int(100*B)))
xs_our=np.mean(xs_our,axis=1).T.tolist()[0]
ts_our=load_dict('E_Epinions_ts_our_'+str(int(100*B))).tolist()[0]
ts_our=[i-200 for i in ts_our]
xs_degree=load_dict('E_Epinions_xs_degree_'+str(int(100*B)))
xs_degree=np.mean(xs_degree,axis=1).T.tolist()[0]
ts_degree=load_dict('E_Epinions_ts_degree_'+str(int(100*B))).tolist()[0]
ts_degree=[i-200 for i in ts_degree]
xs_laplacian=load_dict('E_Epinions_xs_laplacian_'+str(int(100*B)))
xs_laplacian=np.mean(xs_laplacian,axis=1).T.tolist()[0]
ts_laplacian=load_dict('E_Epinions_ts_laplacian_'+str(int(100*B))).tolist()[0]
...
# plt.yscale('log')
# plt.axis('equal')
plt.savefig('E_Epinions_state_'+str(int(100*B))+'.pdf',dpi=300)
plt.show()